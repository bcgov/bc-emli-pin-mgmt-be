...
/*
	...
	Sample command (execute from this folder):
		k6 run -e URL='server url here' -e API_KEY='key here' -e PIN='valid pin in db' -e PIDS='matching pid in db' -e VERIFY_IDEAL_TARGET=20 -e VERIFY_SPIKE_TARGET=50 verify.js
*/
...
export let options = {
    ...
    scenarios: {
        ideal: {
            ...
            startTime: '70s', // to allow the spike to finish. Otherwise these run at the same time
        },
        spike: {
            executor: 'ramping-vus',
            startVUs: 0,
...
export default function () {
    ...
    let body = { pin: __ENV.PIN, pids: __ENV.PIDS };
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    let res = http.post(__ENV.URL, JSON.stringify(body), { headers: headers });
